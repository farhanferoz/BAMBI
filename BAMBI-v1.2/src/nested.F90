...
contains
  ...
  nest_updInt,nest_Ztol,nest_root,seed,nest_pWrap,nest_fb,nest_resume,nest_outfile,initMPI,nest_logZero,nest_maxIter,loglike, &
  dumper,bambi,context)
	...
	call Nestsample(loglike, dumper, bambi, context)
	deallocate(pWrap)
      	call killRandomNS()
#ifdef MPI
...
!----------------------------------------------------------------------
  ...
  subroutine Nestsample(loglike, dumper, bambi, context)
	...
	call clusteredNest(p,phyP,l,loglike,dumper,bambi,context)
...
!----------------------------------------------------------------------
  ...
  subroutine clusteredNest(p,phyP,l,loglike,dumper,bambi,context)
	...
	double precision, pointer :: BAMBIData(:,:)
	...
	double precision, allocatable :: pnew(:), phyPnew(:), pnewunscaled(:) ! new point
	...
	allocate( evData(updInt,totPar+3), BAMBIData(updInt,ndims+1) )
	...
	allocate( pnew(ndims), phyPnew(totPar), pnewunscaled(ndims) )
	...
	do ff=1,maxIter
		...
		if(ic_done(0)) then
			...
			deallocate( eswitchff, escount, dmin, evData, BAMBIData, ic_sc, ic_npt, ic_done, ic_climits, &
			ic_volFac, sc_mean, sc_tmat, sc_kfac, sc_eff, sc_vol, lowp, lowphyP, pnew, phyPnew, pnewunscaled )
			return
		endif
			...
			if(.not.ic_done(0)) then
				...
				if(.not.eswitch) then
					...
					enddo
	                        	...
	                        	if(my_rank==0) then      	
	                  			p(:,indx(1))=pnew(:)
						pnewunscaled(:)=ic_climits(nd,:,1)+(ic_climits(nd,:,2)-ic_climits(nd,:,1))*pnew(:)
	                  			phyP(:,indx(1))=phyPnew(:)
	                  			l(indx(1))=lnew
...
!					(ic_info(nd)+gZold)-ic_Z(nd)
					...
					!BAMBIData(j1,1:ndims)=lowP(1:ndims)
					!BAMBIData(j1,ndims+1)=lowlike
					!BAMBIData(j1,1:ndims)=pnew(1:ndims)
					!BAMBIData(j1,ndims+1)=lnew
					do j=1,updint-1
						BAMBIData(j,:)=BAMBIData(j+1,:)
					enddo
					BAMBIData(updint,1:ndims)=pnewunscaled(1:ndims)
					BAMBIData(updint,ndims+1)=lnew
...
#ifdef MPI
			call MPI_BARRIER(MPI_COMM_WORLD,errcode)
			call MPI_BCAST(j1,1,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
			call MPI_BCAST(sff,1,MPI_INTEGER,0,MPI_COMM_WORLD,errcode)
#endif
			if(sff>=updint .and. mod((j1*2),updInt)==0) call bambi(updInt, ndims, BAMBIData, lowlike)
	...
	deallocate( eswitchff, escount, dmin, evData, BAMBIData, ic_sc, ic_npt, ic_done, &
