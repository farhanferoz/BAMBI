...
#include "NNopt.h"
       ...
       #define GETLOGLIKE like_mp_getloglikens_
       #define GETPHYSPARAMS like_mp_getphysparams_
       #define GETALLPARAMS like_mp_getallparams_
       #define GETMNPARAMS nestwrapper_mp_getmnparams_
#elif defined __GNUC__ 				// if the MultiNest library was compiled with gfortran
       #define GETLOGLIKE __like_MOD_getloglikens
       #define GETPHYSPARAMS __like_MOD_getphysparams
       #define GETALLPARAMS __like_MOD_getallparams
       #define GETMNPARAMS __nestwrapper_MOD_getmnparams
#else
       #error Do not know how to link to Fortran libraries, check symbol table for your platform (nm libnest3.a | grep nestrun) & edit example_gauss_shell/bambi.cc
#endif
...
int doBAMBI=0,useNN=0,whitenin,whitenout;
double logzero;
...
namespace nestwrap
{
	...
	extern "C" void GETPHYSPARAMS(double *Cube);
	extern "C" void GETALLPARAMS(double *Cube);
	extern "C" void GETMNPARAMS(char *root, char *netfile, int &resume, double &logzero, int &do_bambi, int &use_NN);
	...
	void getphysparams(double *Cube)
	{
		GETPHYSPARAMS(Cube);
	}

	void getallparams(double *Cube)
	{
		GETALLPARAMS(Cube);
	}

	void getmnparams(char *root, char *netfile, int &resume, double &logzero, int &do_bambi, int &use_NN)
	{
		GETMNPARAMS(root, netfile, resume, logzero, do_bambi, use_NN);
		...
		for(int i = 0; i < strlen(netfile); i++)
		{
			if( netfile[i] == ' ' )
			{
				netfile[i] = '\0';
...
extern "C" void getloglikec_(double *Cube, int &ndim, int &npars, double &lnew, void *context)
{
...
#endif
		int resvar;
		nestwrap::getmnparams(root,networkinputs,resvar,logzero,doBAMBI,useNN);
		totpar=npars;
	...
	{
		nestwrap::loglike(Cube, ndim, npars, lnew, context);
		return;
	}
	else if( useNN==1 || useNN==2 || ( doBAMBI==1 && converged ) )
	{
		nestwrap::getphysparams(Cube);
	}
	...
	if( doBAMBI==1 && converged )
	{
		...
		if( netL>thL[1] )
		{
			...
			double DnetL;
			nestwrap::loglike(Cube, ndim, npars, DnetL, context);
			netL=(float)DnetL;
			/*converged=false;
			thcounter[0] = thcounter[1] = 0;*/
		}
...
extern "C" void bambi_(int &ndata, int &ndim, double **BAMBIData, double &lowlike)
{
