...
! Parameters for Nested Sampler
	...
	parameter(nest_root='chains/gauss_shell_2_')
      
      	!root for saving posterior files
      	character*100 nest_netfile
	parameter(nest_netfile='example_gauss_shell/gauss_shell_net.inp')
	
	!after how many iterations feedback is required & the output files should be updated
	!note: posterior files are updated & dumper routine is called after every updInt*10 iterations
	integer nest_updInt
	parameter(nest_updInt=2000)
	...
	integer nest_pWrap(sdim)
      	...
      	!do BAMBI?
      	logical nest_bambi
      	parameter(nest_bambi=.true.)
	
	!use previously trained NN?
	!0=none, 1=slow method, 2=fast method
	integer nest_useNN
	parameter(nest_useNN = 0)
